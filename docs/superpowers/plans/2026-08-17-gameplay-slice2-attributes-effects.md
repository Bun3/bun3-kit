...
- 수정자 크기의 Operand는 **적용 시점 평가(스냅샷)로 고정** — 적용 후 원본 속성이 변해도 이미 적용된 수정자 크기는 불변 (스펙 §4 보완 결정, 결정론·단순성 우선. live 갱신은 수요 시 후속).
...
**Interfaces:**
- Produces: `Operand`(readonly struct — `Kind: OperandKind`, `Value: BigNum`, `AttributeId: ushort`; 팩토리 `Operand.Constant(BigNum)`, `Operand.Attribute(ushort)`, `Operand.Attribute(ushort, BigNum coefficient)`; `IEquatable<Operand>`), `OperandKind { Constant=0, Attribute=1 }`, `AttributeModifierOp { Add=0, Multiply=1, Override=2 }`, `MaxIncreasePolicy { Stay=0, Follow=1 }`, `MaxDecreasePolicy { Follow=0, Stay=1 }`, `ComparisonOp { Equal, NotEqual, Less, LessOrEqual, Greater, GreaterOrEqual }`.
...
7. 모든 Operand의 속성 참조는 AttributeRegistry에 존재해야 함.
...
- Produces:
  ...
  - 크기 평가(내부 정적): `Operand` 평가 = Constant→값, Attribute→대상(또는 소스 아님 — **v1: 속성 참조는 대상(target) 속성**으로 고정, 소스 참조는 MagnitudeCalc로) × 계수. `MagnitudeDef` 평가 = Calc 있으면 Calc.Calculate, 아니면 `Base + PerLevel×(level-1)`.
...
```
  ...
  피연산자 JSON: `{"constant":"50"}` | `{"attribute":"Hp","coefficient":"0.3"}`. magnitude에 추가로 `{"calc":"calc.magnitude.x"}` | `{"base":{...},"perLevel":{...}}`. BigNum 리터럴은 항상 문자열(JSON number 금지 — double 경유 차단).
...
- 미결 주의점(구현자가 알아야 할 것): ① Modifier 크기의 Operand 속성 참조는 **대상(target) 속성**이다 — 소스 속성 기반 크기는 MagnitudeCalc로(Global Constraints의 스냅샷 평가와 함께 §4 보완 결정). ② `EffectTestKit`은 T7에서 만들고 T8~13이 확장한다 — 시그니처 변경 시 이전 태스크 테스트도 같이 컴파일되므로 주의.
