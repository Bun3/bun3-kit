    ...
    public sealed class SoundProfileTests
    {
        ...
        public void SpatialAndRoutingProfiles_ReplaceDefaultsWithoutOverwritingLocalSettings()
        {
            ...
            try
            {
                ...
                def.AttenuationProfile = attenuation;
                def.Occlusion = true;
                def.OcclusionVolumeAtFull = 0.2f;
                def.InheritSpatialBlend = false;
                def.SpatialBlendProfile = blend;
                def.MonoDistance = 2f;
                ...
                Assert.That(def.EffectiveMinDistance, Is.EqualTo(5f));
                Assert.That(def.EffectiveMaxDistance, Is.EqualTo(60f));
                Assert.That(def.EffectiveDistanceAttenuation, Is.False);
                Assert.That(def.EffectiveAttenuationProfile, Is.SameAs(attenuation));
                Assert.That(def.EffectiveOcclusion, Is.True);
                Assert.That(def.EffectiveOcclusionVolumeAtFull, Is.EqualTo(0.2f));
                Assert.That(def.EffectiveInheritSpatialBlend, Is.False);
