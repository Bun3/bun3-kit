...
- Declare SerializeReference Extensions as a version dependency; consuming projects select its Git source in their manifest.
...
- The work-area fit now targets the primary monitor by default
  (`MonitorFit.FitToPrimary`, on) — the OS or Unity can place the window on a
  secondary display at startup, and fitting "wherever it happens to be" kept the
  overlay there. Set `FitToPrimary = false` for the old current-monitor behavior.
