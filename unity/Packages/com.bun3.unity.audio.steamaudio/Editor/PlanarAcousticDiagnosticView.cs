...
namespace Bun3.Unity.Audio.SteamAudio.Editor
{
    ...
    public sealed class PlanarAcousticDiagnosticView
    {
        ...
        public void DrawInspector(PlanarAcousticDiagnostics.Snapshot snapshot)
        {
            ...
            {
                DrawVoiceDetails(voice);
            }
            EditorGUILayout.EndScrollView();
        }

        static void DrawVoiceDetails(Voice voice)
        {
        ...
        static void DrawCurve(Voice voice)
        {
            ...
            var points = BuildCurvePoints(voice, rect, end);
            Handles.BeginGUI();
        ...
        static Vector3[] BuildCurvePoints(Voice voice, Rect rect, float end)
        {
            var points = new Vector3[129];
            for (int i = 0; i < points.Length; i++)
            {
                float distance = end * i / (points.Length - 1);
                voice.evaluation ??= new Bun3.Unity.Audio.DistanceAttenuationCurve(voice.curve);
                float gain = voice.attenuationEnabled ? voice.evaluation.Evaluate(distance) : 1;
        ...
        static void DrawNativePath(PlanarAcousticDiagnostics.NativePath path)
        {
